...
async function checkpoint(warmup=false){phase('BREAK');stage.innerHTML=`<div class="checkpoint-card"><h1>${warmup?'Practice complete':`Round ${state.block-1} complete`}</h1><p>Next AI adviser: ${esc(state.adviser_name||'AI')}</p><p>Take a break if you like.</p><button class="button primary" id="continue-round">Start round ${state.block}</button></div>`;const t=clock();await new Promise(r=>document.getElementById('continue-round').onclick=r);timing.break_ms=elapsed(t).wall;}
...
async function showAdvice(initial,advice) {
  ...
  if(CFG.advice_preview_ms>0){
    ...
    stage.innerHTML=`<section class="advice-only ai-colour"><p>${esc(advice.adviser_name && advice.adviser_name!=='AI' ? advice.adviser_name+' · AI ADVICE' : 'AI ADVICE')}: <strong>${esc(advice.advice_number)}</strong></p><h1>“${esc(advice.advice_text)}”</h1></section>`;
    try{
      await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
...
async function ratings(){
  ...
  stage.innerHTML=`<div class="rating-stage"><form id="rating-form">${scale('trust',`How much did you trust ${state.adviser_name||'the AI'} over the last ${state.rating_window} trials?`,'Not at all','Completely')}${paired?scale('feeling','How did the AI advice make you feel?','Very negative','Very positive'):''}</form></div>`;
...
if(state.pending){phase('WELCOME BACK');stage.innerHTML=`<div class="checkpoint-card"><h2>Your last answer is saved.</h2><p>Continue with the advice for that trial.</p><button class="button primary" id="resume-advice">Continue →</button></div>`;await new Promise(r=>document.getElementById('resume-advice').onclick=r);initial={estimate:state.pending.initial,active:state.pending.rt_initial,wall:0};advice={adviser_name:state.adviser_name,advice_text:state.pending.text,advice_number:state.pending.advice};}
