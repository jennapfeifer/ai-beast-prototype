...
async function showStimulus(){phase('LOOK AT THE DOTS');const prefetchStarted=performance.now();prefetchPromise=fetchJSON('/api/prefetch',{trial_token:state.trial_token}).then(result=>{timing.prefetch_request_ms=Math.round(performance.now()-prefetchStarted);return result;}).catch(()=>null);const start=clock();stage.innerHTML=`<div class="stimulus-stage"><img id="stimulus" class="stimulus" width="512" height="512" src="${esc(state.image)}" alt="A field of dots to estimate"></div>`;const image=document.getElementById('stimulus');await image.decode();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));timing.stimulus_load_ms=elapsed(start).wall;const bounds=image.getBoundingClientRect();timing.stimulus_render_width=bounds.width;timing.stimulus_render_height=bounds.height;
  ...
  return new Promise(r => {
    stage.innerHTML = `
      <div class="initial-card stage-enter">
        ...
        <h1 class="game-question">How many dots?</h1>
        <p class="game-subtitle">Click the line or type an estimate.</p>
        ...
        <div id="initial-readout" class="initial-readout muted">Choose an estimate</div>
        <div class="center"><button class="button primary" id="go" disabled>Continue</button></div>
  ...
  return new Promise(r => {
    ...
    stage.innerHTML = `
      <div class="decision-card stage-enter">
        ...
        <h1 class="game-question">Your final estimate</h1>
        ...
        <p class="slider-hint">Keep or adjust your estimate.</p>
        <div class="center"><button class="button primary" id="go">Confirm</button></div>
      </div>`;
