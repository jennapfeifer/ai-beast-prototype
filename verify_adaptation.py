...
def histories(direction, contrast='behaviour'):
    ...
    if contrast == 'trust':
        ...
        return {name: [dict(row, trust_rating=rating if row['trust_rating'] is not None else None,
                           feeling_rating=4 if row['feeling_rating'] is not None else None)
                       for row in result['followed']]
                for name, rating in [('low_trust',1),('high_trust',7)]}
    if contrast != 'behaviour':
        raise ValueError('contrast must be behaviour or trust')
    return result
...
def run_probe(live=False, repetitions=1, contrast='behaviour'):
    ...
    return dict(created_at=dt.datetime.now(dt.timezone.utc).isoformat(), mode='live' if live else 'offline',
        ...
        summary=dict(items=len(results), routing_passed=all(r['passed'] for r in routing),
            ...
            meaningfully_adaptive='NOT AUTOMATICALLY ASSESSED', trust_effect='NOT AUTOMATICALLY ASSESSED', behavioural_effect='NOT TESTED'),
...
def write_report(report, folder):
    ...
    cards = ''.join(f'<article><small>{esc(r["item_id"])} · {esc(r["source"])} · {r["elapsed_ms"]} ms</small>'
        f'<blockquote>{esc(r["text"])}</blockquote><p>Validation: {esc(r["validation"])}; attempts: {r["attempts"]}; trust check: {esc(r.get("trust_check", "not_checked_offline"))}</p>'
    ...
    comparison = 'low vs high reported trust, with behaviour held constant' if report['contrast']=='trust' else 'resisted vs followed'
...
def main():
    ...
    parser.add_argument('--contrast', choices=['behaviour','trust'], default='behaviour')
